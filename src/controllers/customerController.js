...
class CustomerController {
   ...
   */
  async unblockSales(req, res, next) {
    try {
      const customer = await Customer.findOne({ _id: req.params.id, ...req.tenantFilter });
      if (!customer) return next(AppError.notFound('العميل غير موجود'));
   ...
   */
  async getCreditAssessment(req, res, next) {
    try {
      const customer = await Customer.findOne({ _id: req.params.id, ...req.tenantFilter });
      if (!customer) return next(AppError.notFound('العميل غير موجود'));

      // Recalculate credit score
      customer.calculateCreditScore();
      await customer.save();
   ...
   */
  async getStatementPDF(req, res, next) {
    try {
      const customer = await Customer.findOne({ _id: req.params.id, ...req.tenantFilter });
      if (!customer) return next(AppError.notFound('العميل غير موجود'));
   ...
   */
  async sendStatementPDF(req, res, next) {
    try {
      const customer = await Customer.findOne({ _id: req.params.id, ...req.tenantFilter });
      if (!customer) return next(AppError.notFound('العميل غير موجود'));
      ...
      const templateResult = await WhatsAppService.sendStatementTemplate(customer.phone, customer);
      ...
      if (templateResult.success) {
        ...
        return ApiResponse.success(res, {
          ...
          whatsappSent: true,
        }, 'تم إرسال كشف الحساب عبر WhatsApp Template ✅');
      }

      // Template failed, try sending PDF document
      logger.info(`[Statement] Template failed, trying PDF... (${templateResult.error?.message || 'Unknown error'})`);
      ...
      if (!whatsappResult.success) {
        ...
        const textResult = await WhatsAppService.sendMessage(customer.phone, message);
