...
class BusinessIntelligenceController {
  ...
  async getCommandCenter(req, res, next) {
    try {
      ...
      }).populate('supplier', 'name phone').select('name sku stock stockStatus supplier').lean();
      ...
      if (lowStockProducts.filter(p => p.stockStatus === 'out_of_stock').length > 0) {
        suggestions.push({
          priority: 'high',
          icon: '📦',
          text: `${lowStockProducts.filter(p => p.stockStatus === 'out_of_stock').length} منتج نفذ — اطلب من الموردين`,
          action: 'restock',
        });
      }
   ...
   */
  async whatIfSimulator(req, res, next) {
    try {
      const tenantId = new mongoose.Types.ObjectId(req.tenantId);
      ...
      if (scenario === 'price_change') {
        ...
        if (!product) return next(new Error('المنتج غير موجود'));
